...
declare_id!("CBPwzbexyoGWHS2LfPHekGdCjNMZYBjZgeynLT7g5qzv");

pub fn transfer<'a>(
    system_program: AccountInfo<'a>, // Pass the system program account
    from: AccountInfo<'a>,           // Account to transfer from
    to: AccountInfo<'a>,             // Account to transfer to
    ...
    // Create a new context with the system program and the transfer accounts
    let transfer_ctx = match seeds {
        // If seeds are present, create a new context with the system program,
        // transfer accounts, and seeds
    ...
    // Call the system program's transfer function
...
pub mod sb_randomness {
    ...
    pub fn coin_flip(
        ctx: Context<CoinFlip>,     // Context with the CoinFlip accounts
        randomness_account: Pubkey, // The Switchboard randomness account
        guess: bool,                // The user's guess
    ) -> Result<()> {
        let clock = Clock::get()?;
        let player_state = &mut ctx.accounts.player_state;
